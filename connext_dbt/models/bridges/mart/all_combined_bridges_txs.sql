...
(
    SELECT
        across.bridge,
        CONCAT("across", "_", ROW_NUMBER() OVER (ORDER BY across.from_date DESC)) AS id,
        across.from_date,
        across.from_tx_hash,
    ...
    ORDER BY across.from_date DESC
)
...
(
    SELECT
        all_bridge.bridge,
        CONCAT("all_bridge", "_", ROW_NUMBER() OVER (ORDER BY all_bridge.from_date DESC)) AS id,
    ...
    ORDER BY all_bridge.from_date DESC
)
...
(
    SELECT
        debridge.bridge,
        CONCAT("debridge", "_", ROW_NUMBER() OVER (ORDER BY debridge.from_date DESC)) AS id,
        debridge.from_date,
    ...
    ORDER BY debridge.from_date DESC
)
...
(
    SELECT
        ...
        hop.bridge,
        CONCAT("hop", "_", ROW_NUMBER() OVER (ORDER BY hop.from_date DESC)) AS id,
        hop.from_date,
        hop.from_tx_hash,
        hop.from_chain_id,
        hop.from_chain_name,
...
(
    SELECT
        stargate.bridge,
        CONCAT("stargate", "_", ROW_NUMBER() OVER (ORDER BY stargate.from_date DESC)) AS id,
        stargate.from_date,
    ...
    ORDER BY stargate.from_date DESC
)
...
(
    SELECT
        synapse.bridge,
        CONCAT("synapse", "_", ROW_NUMBER() OVER (ORDER BY synapse.from_date DESC)) AS id,
        synapse.from_date,
    ...
    ORDER BY synapse.from_date DESC
)