...
WITH
  ...
  SELECT
    ...
    (CAST(lr.route_toamountusd AS FLOAT64) - CAST(lr.route_gascostusd AS FLOAT64)) AS receivedvalueinusd,
  ...
  SELECT
    ...
    IFNULL(
    REGEXP_REPLACE(REGEXP_EXTRACT(usedbridgenames, r'\[(.*?)\]'), r"'", ''),
    usedbridgenames
    )
    AS usedbridgenames,
    totalusertx
  ...
  SELECT
    *,
    RANK() OVER (PARTITION BY date, aggregator, route_fromchainid, route_tochainid, route_fromtoken_symbol, route_totoken_symbol ORDER BY receivedvalueinusd DESC ) AS max_value_rank_by_output
  FROM
    raw ),
