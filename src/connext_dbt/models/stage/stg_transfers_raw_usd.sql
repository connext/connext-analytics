...
, semi_txs AS (
  SELECT
    ...
    coalesce(cc_origin.token_name, t.origin_transacting_asset) AS origin_asset,
    CAST(cc_destination.is_xerc20 AS BOOL) AS is_destination_asset_xerc20,
    coalesce(cc_destination.token_name, t.destination_transacting_asset) AS destination_asset,
...
SELECT
  ...
  CAST(sr.origin_transacting_amount AS FLOAT64) / POW(10, COALESCE(CAST(a_origin.adopted_decimal AS INT64), 0)) AS d_origin_amount,
  CAST(sr.bridged_amt AS FLOAT64) / POW(10, COALESCE(CAST(a_origin.decimal AS INT64), 0)) AS d_bridged_amt,
  CAST(sr.destination_transacting_amount AS FLOAT64) / POW(10, COALESCE(CAST(a_dest.adopted_decimal AS INT64), 0)) AS d_destination_amount,
  --USD amounts
  (CAST(sr.origin_transacting_amount AS FLOAT64) / POW(10, COALESCE(CAST(a_origin.adopted_decimal AS INT64), 0))) * sr.price AS usd_origin_amount,
  (CAST(sr.bridged_amt AS FLOAT64) / POW(10, COALESCE(CAST(a_origin.decimal AS INT64), 0))) * sr.price AS usd_bridged_amount,
  (CAST(sr.destination_transacting_amount AS FLOAT64) / POW(10, COALESCE(CAST(a_dest.adopted_decimal AS INT64), 0))) * sr.price AS usd_destination_amount
FROM semi_txs sr
LEFT JOIN `mainnet-bigq.public.assets` a_origin
ON ( sr.canonical_id = a_origin.canonical_id AND sr.origin_domain = a_origin.domain)
LEFT JOIN `mainnet-bigq.public.assets` a_dest
ON ( sr.canonical_id = a_dest.canonical_id AND sr.destination_domain = a_dest.domain)