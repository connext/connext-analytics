...
GCS_FOLDER_NAME = "mode"
TRANSACTION_URL_TEMPLATE = "https://explorer.mode.network/api/v2/transactions/{}"
...
def convert_json_to_df_and_upload_to_bq(json_list: list) -> None:
    ...
    pandas_gbq.to_gbq(
        ...
        destination_table="stage.source_mode_weth_arb_chain_deposits__transactions",
        if_exists="append",
        chunksize=100000,
        api_method="load_csv",
    )
    logging.info(f"Mode WETH Arb chain Deposits, {df.shape} rows Added!")
...
async def fetch_transaction_data(
    ...
    """
    Fetches transaction data from the Mode network explorer API for a given transaction ID.
    Retries the request with exponential backoff in case of failures.

    curl eg:
        curl -X 'GET' \
        'https://explorer.mode.network/api/v2/transactions/0xaa148281d42070e53674336582d77a05a8fef1c33d70f33b97d5cb438c6f8ef4' \
...
async def main_fetch(parallel_fetch=10):
    ...
    # get txs as list
    df = get_raw_from_bq(sql_file_name="transfers_list_origin_mode_arb_deposits")
    if df.empty:
        logging.info(
            """No new transactions found to pull data! Data is upto date in
            stage.source_mode_weth_arb_chain_deposits__transactions"""
        )
        return None
...
if __name__ == "__main__":
    asyncio.run(main_fetch(parallel_fetch=10))
