...
WITH last_transfer_timestamp AS (
    SELECT max(xcall_timestamp) AS last_transfer_update_date
    FROM `mainnet-bigq.public.transfers_in_usd`
),
...
transfer_raw AS (
    SELECT
        ...
        cast(timestamp_seconds(t.execute_timestamp) AS STRING)
            AS execute_timestamp,
        cast(timestamp_seconds(t.reconcile_timestamp) AS STRING)
            AS reconcile_timestamp,
        CASE
        ...
        cc.contract_name,
        cc.contract_author,
        coalesce(cc_origin.token_address, t.origin_transacting_asset)
            AS origin_asset,
        coalesce(
            cc_destination.token_address, t.destination_transacting_asset
        ...
        / pow(10, coalesce(cast(a_dest.decimal AS INT64), 0))
...
transfers_usd_price AS (
    ...
    LEFT JOIN hr_asset_price ap
        ON (
            ...
            AND ap.timestamp
            <= (tr.xcall_timestamp - mod(tr.xcall_timestamp, 1800))
        )
)
...
SELECT
    ...
    tsp.d_destination_amount
    * cast(tsp.price AS FLOAT64) AS usd_destination_amount
