...
final_6hr_all AS (
    SELECT DATE_TRUNC('day', six_hour_interval) AS day,
        from_chain_id,
        from_asset_address,
        from_asset_symbol,
        to_chain_id,
...
final_all AS (
    SELECT COALESCE(final_6hr_all.day, final_24hr_all.day) AS day,
        COALESCE(
...
final_1hr_mm AS (
    SELECT DATE_TRUNC('day', one_hour_interval) AS day,
        from_chain_id,
        from_asset_address,
        from_asset_symbol,
        to_chain_id,
...
final_3hr_mm AS (
    SELECT DATE_TRUNC('day', three_hour_interval) AS day,
        from_chain_id,
        from_asset_address,
        to_chain_id,
        to_asset_address,
...
,
mm_final AS (
    SELECT COALESCE(final_1hr_mm.day, final_3hr_mm.day) AS day,
        COALESCE(
...
SELECT COALESCE(final_all.day, mm_final.day) AS day,
...
FROM final_all
    FULL OUTER JOIN mm_final ON final_all.day = mm_final.day
