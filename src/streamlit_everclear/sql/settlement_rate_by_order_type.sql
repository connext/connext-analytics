...
SELECT 
    ...
    COUNT(CASE
        WHEN (
            COALESCE(CAST(i.hub_settlement_enqueued_timestamp AS FLOAT), CAST(i.settlement_timestamp AS FLOAT)) - i.origin_timestamp) < 21600
        THEN i.id
        ELSE NULL
...
, final_6hr_all AS (
SELECT 
    DATE_TRUNC('day', six_hour_interval) AS day,
    from_chain_id,
    from_asset_address,
    to_chain_id,
    to_asset_address,
...
SELECT 
    ...
    COUNT(CASE
        WHEN (
            COALESCE(CAST(i.hub_settlement_enqueued_timestamp AS FLOAT), CAST(i.settlement_timestamp AS FLOAT)) - i.origin_timestamp) < 86400
        THEN i.id
        ELSE NULL
...
, final_all AS (
SELECT 
    COALESCE(final_6hr_all.day, final_24hr_all.day) AS day,
...
FROM final_6hr_all
FULL OUTER JOIN final_24hr_all ON final_6hr_all.day = final_24hr_all.day
    AND final_6hr_all.from_chain_id = final_24hr_all.from_chain_id
...
SELECT
    ...
    -- 1hr
    COUNT(CASE
        WHEN ((inv.hub_invoice_enqueued_timestamp::float - inv.origin_timestamp::float)) < 3600
        THEN inv.id
        ELSE NULL
...
, final_1hr_mm AS (
SELECT 
    DATE_TRUNC('day', one_hour_interval) AS day,
    from_chain_id,
    from_asset_address,
    to_chain_id,
    to_asset_address,
...
SELECT
    ...
    -- 3hr
    COUNT(CASE
        WHEN ((inv.hub_invoice_enqueued_timestamp::float - inv.origin_timestamp::float)) < 10800
        THEN inv.id
        ELSE NULL
...
, mm_final AS (
SELECT 
    COALESCE(final_1hr_mm.day, final_3hr_mm.day) AS day,
...
FROM final_1hr_mm
FULL OUTER JOIN final_3hr_mm ON final_1hr_mm.day = final_3hr_mm.day
    AND final_1hr_mm.from_chain_id = final_3hr_mm.from_chain_id
...
SELECT 
    COALESCE(final_all.day, mm_final.day) AS day,
...
FULL OUTER JOIN mm_final 
    ON final_all.day = mm_final.day
    AND final_all.from_chain_id = mm_final.from_chain_id
